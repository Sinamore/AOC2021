...
with open('tests/18_2.input', 'r') as inp:
    numbers = inp.read()[:-1].split('\n')
    numbers_t = [make_tree(n) for n in numbers]

    mx = 0
    for i in range(len(numbers_t)):
        for j in range(i + 1, len(numbers_t)):
            t = tree_reduce(tree_add(copy.deepcopy(numbers_t[i]),
                                     copy.deepcopy(numbers_t[j])))
            mx = max(mx, tree_magnitude(t))
            t = tree_reduce(tree_add(copy.deepcopy(numbers_t[j]),
                                     copy.deepcopy(numbers_t[i])))
            mx = max(mx, tree_magnitude(t))
